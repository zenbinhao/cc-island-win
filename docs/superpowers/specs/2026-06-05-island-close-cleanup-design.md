...
唯一**改写**历史行为的点：上个设计保留「最后一行消除后留 52px 空壳窗」，本设计改为「**空了即隐藏窗口**」（用户本次明确选「最后一个关了整窗消失」，且统一到所有摘行路径——含手动 ×）。详见「整窗隐藏」一节与「已决议」。
...
1. **手动 `×` 消除最后一行时也整窗隐藏（已确认）**：统一「空了即隐藏」，无论摘行原因（关 CC / 探活 / 手动 ×）。语义一致：空看板=隐藏，下次有事件再现。**改写**上个设计「× 最后一行留 52px 空壳」的行为 → `removeRowById` 空判隐藏对所有摘行路径生效（含现有 `dismiss` 分支）。

## 实现期注记

- **隐藏手段先 B 后 A**：先用 B（`resize(WIN_W, 0)`，不动 C#）；若实测 0 高仍露可见痕迹，再改走 A（host 加 `hide/show` 命令 + 重编 exe + 提交二进制）。非用户决策，实现期按实测定。
