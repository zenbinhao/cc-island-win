...
点击跳转链路：`UserPromptSubmit` 时 bridge 在 update 上标记 `captureFg` → companion 让常驻原生窗口捕获当时的前台窗口 HWND（用户刚按回车，前台即该终端）→ 点击胶囊行 → 原生窗口 `SetForegroundWindow` 拉起对应终端（已最小化则先还原）。零额外进程、不探测终端类型；粒度为窗口级（同窗多 pane 无法区分）。
...
跳转依赖该会话**提交过 prompt**（UserPromptSubmit 时刻捕获前台窗口句柄）。若会话从未提交过 prompt、或对应终端窗口已关闭，点击行不会有动作（静默忽略）。同一终端窗口内多个 pane 无法区分——跳转只到窗口级。
...
- **点击跳转**: 点击某行（× 以外任意位置）把该会话所在终端窗口拉到前台，已最小化会先还原；hover 时行高亮并淡入 ↗ 提示。窗口级粒度，同窗多 pane 无法区分；会话尚无 UserPromptSubmit 捕获时点击无效果。
