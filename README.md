...
- **点击跳转**: 点击某行（× 以外的任意位置）即把该会话所在的终端窗口拉到前台（已最小化也会还原）。原理：每次提交 prompt 时由常驻原生窗口捕获当时的前台窗口句柄，零额外进程。限制：窗口级粒度——同一终端窗口里的多个 pane 无法区分；该会话尚未提交过 prompt 时点击无效果
...
- **收起/展开**: 点击底部手柄（chevron）手动收起/展开；收起后仅显示手柄；有新状态更新时自动展开
- **多会话**: 每个 Claude Code 实例独立显示；行进出场为平滑滑动动画，waiting/done 行有呼吸光强调
- **尺寸**: 基准行 540×40（medium），`/island scale` 四档缩放，窗口宽高随档位自适应
...
- **DPI / 多屏**: 原生窗口 PerMonitorV2 DPI 感知（高缩放屏不模糊、点击热区不偏移）；分辨率/显示器拓扑变化后自动重新归位；屏幕选择（`/island screen`）由原生窗口自行解析，不再依赖 PowerShell
